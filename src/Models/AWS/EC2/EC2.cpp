...
void Instance::setGpuFromSpecification(int vram) {
  ...
  if (vram != 1 && vram != 2 && vram != 4 && vram != 8) {
    std::cout
        << "Your GPU VRAM value is invalid. Will match to the closest value..."
        << std::endl;
  }
    ...
    if (osName.find("ubuntu") != std::string::npos) {
      if (osName.find('@')) {
        std::vector<std::string> split =
            Helpers::HelperFunctions::splitStringByDelimiter(osName, '@');
        ...
        if (this->ImageId == "") {
          std::cout << "Ubuntu " << version
                    << " is not available in this region. Setting to 18.04..."
                    << std::endl;
          this->ImageId = j.at("ubuntu1904").at(this->AvailabilityZone);
        }
      } else {
      ...
      if (this->ImageId == "") {
        std::cout << "FreeBSD is not available in this region. Setting to "
                     "amazon os..."
                  << std::endl;
      ...
      if (osName.find('@')) {
        ...
        if (this->ImageId == "") {
          std::cout
              << "CentOS " << version
              << " is not available in this region. Setting to Amazon Linux..."
              << std::endl;
      ...
      if (osName.find('@')) {
        ...
        } else if (version.rfind("2016", 0) == 0 ||
                   version.rfind("16", 0) == 0) {
          this->ImageId = j.at("windows2016").at(this->AvailabilityZone);
        } else if (version.rfind("2012r2", 0) == 0 ||
                   version.rfind("12r2", 0) == 0 ||
                   version.rfind("2012 r2", 0) == 0 ||
                   version.rfind("12 r2", 0) == 0) {
          this->ImageId = j.at("windows2012R2").at(this->AvailabilityZone);
        } else if (version.rfind("2012", 0) == 0 ||
                   version.rfind("12", 0) == 0) {
        ...
        if (this->ImageId == "") {
          std::cout << "Windows " << version
                    << " is not available in this region. Setting to Windows "
                       "Server 2019..."
                    << std::endl;
          this->ImageId = j.at("windows2019").at(this->AvailabilityZone);
        }
